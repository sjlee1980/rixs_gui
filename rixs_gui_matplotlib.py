...
class RIXSGui(QMainWindow):
    ...
    def show_context_menu(self, position):
        ...
        if not item or not item.data(0, USER_ROLE): return
        fname, sname = item.data(0, USER_ROLE); rixs = self.raw_data[fname][sname]
        msg = f"<b>Sample ID:</b> {rixs.get('sample_id', 'N/A')}<br><b>Sample Name:</b> {rixs.get('sample_name', 'N/A')}"
    ...
    def on_select_roi(self, eclick, erelease):
        x1, x2 = sorted([eclick.xdata, erelease.xdata]); y1, y2 = sorted([eclick.ydata, erelease.ydata]); self.roi_xes_start.setText(f"{x1:.2f}"); self.roi_xes_end.setText(f"{x2:.2f}"); self.roi_pfy_start.setText(f"{y1:.2f}"); self.roi_pfy_end.setText(f"{y2:.2f}"); self.update_integrations()
