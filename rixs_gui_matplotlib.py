...
# Fix for NumPy 2.0 unpickling legacy data
...
class RIXSGui(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("RIXS Analyzer")
    ...
    def init_ui(self):
        ...
        color_layout.addWidget(QLabel("CMap:"), 0, 0); color_layout.addWidget(self.cmap_combo, 0, 1)
        self.slider_min = QSlider(Qt.Orientation.Horizontal); self.slider_min.setRange(0, 100); self.slider_min.setValue(0)
        self.slider_min.valueChanged.connect(self.update_color_scale); color_layout.addWidget(QLabel("Min:"), 1, 0); color_layout.addWidget(self.slider_min, 1, 1)
        self.slider_max = QSlider(Qt.Orientation.Horizontal); self.slider_max.setRange(0, 100); self.slider_max.setValue(100)
        self.slider_max.valueChanged.connect(self.update_color_scale); color_layout.addWidget(QLabel("Max:"), 2, 0); color_layout.addWidget(self.slider_max, 2, 1)
        ...
        self.roi_pfy_start = QLineEdit(); self.roi_pfy_start.editingFinished.connect(self.update_integrations_from_text)
        self.roi_pfy_end = QLineEdit(); self.roi_pfy_end.editingFinished.connect(self.update_integrations_from_text)
        roi_pfy_layout.addWidget(QLabel("Start:"), 0, 0); roi_pfy_layout.addWidget(self.roi_pfy_start, 0, 1)
        roi_pfy_layout.addWidget(QLabel("End:"), 1, 0); roi_pfy_layout.addWidget(self.roi_pfy_end, 1, 1)
        ...
        self.roi_xes_start = QLineEdit(); self.roi_xes_start.editingFinished.connect(self.update_integrations_from_text)
        self.roi_xes_end = QLineEdit(); self.roi_xes_end.editingFinished.connect(self.update_integrations_from_text)
        roi_xes_layout.addWidget(QLabel("Start:"), 0, 0); roi_xes_layout.addWidget(self.roi_xes_start, 0, 1)
        roi_xes_layout.addWidget(QLabel("End:"), 1, 0); roi_xes_layout.addWidget(self.roi_xes_end, 1, 1)
        ...
        for f in files:
            try:
                with gzip.GzipFile(f, "rb") as gz:
                    ...
                    except: data = cPickle.load(gz, encoding='latin1')
                ...
                for skey in data.keys():
                    if isinstance(skey, str) and isinstance(data[skey], dict):
                        sample = data[skey].get('sample_name', 'Unknown')
                        folder_item = QTreeWidgetItem(file_item); folder_item.setText(0, f"{skey}: {sample}")
                        folder_item.setData(0, USER_ROLE, (fname, skey))
                        slist = data[skey].get('scan_numbers', str(skey))
                        if isinstance(slist, str): slist = [slist]
                        for snum in slist:
                            scan_item = QTreeWidgetItem(folder_item); scan_item.setText(0, f"Scan {snum}")
    ...
    def on_toggle_counts(self):
        self.clear_overlay() # Resets graphs as requested
    ...
    def capture_current_to_overlay(self):
        ...
        label = selected[0].text(0) if selected else "Initial"
    ...
    def get_summed_data(self):
        ...
        szn, cx, cy, szf, spf = None, None, None, None, None
        for folder_item in items:
            ...
            for i in range(folder_item.childCount()):
                ...
                if sc.checkState(0) == Qt.CheckState.Checked:
                    ...
                    if szn is None: szn, cx, cy, szf, spf = zn.copy(), x, y, zf.copy(), pf.copy()
                    elif zn.shape == szn.shape: szn += zn; szf += zf; spf += pf
        return {'x': cx, 'y': cy, 'z_norm': szn, 'z_flux': szf, 'i0_scale': spf} if szn is not None else None

    def on_tree_selection(self):
        new_data = self.get_summed_data()
        if not new_data: self.clear_overlay(); return
        self.current_data = new_data; d = self.current_data; xmin, xmax = np.min(d['x']), np.max(d['x']); ymin, ymax = np.min(d['y']), np.max(d['y'])
        if not self.roi_pfy_start.text(): self.roi_pfy_start.setText(f"{ymin:.2f}"); self.roi_pfy_end.setText(f"{ymax:.2f}")
        if not self.roi_xes_start.text(): self.roi_xes_start.setText(f"{xmin:.2f}"); self.roi_xes_end.setText(f"{xmax:.2f}")
        ...
        except: ys, ye, xs, xe = ymin, ymax, xmin, xmax
        self.ax_map.clear()
        ...
        self.ax_map.set_xlim(xmin, xmax); self.ax_map.set_ylim(ymin, ymax)
    ...
    def get_integration_indices(self):
        ...
        except: ys, ye = np.min(d['y']), np.max(d['y'])
        return np.argmin(np.abs(d['y'] - ys)), np.argmin(np.abs(d['y'] - ye))

    def get_excitation_indices(self):
        d = self.current_data
        try: xs, xe = float(self.roi_xes_start.text()), float(self.roi_xes_end.text())
        except: xs, xe = np.min(d['x']), np.max(d['x'])
        return np.argmin(np.abs(d['x'] - xs)), np.argmin(np.abs(d['x'] - xe))
    ...
    def update_integrations(self):
        ...
        d = self.current_data; iy_i, iy_f = self.get_integration_indices(); ix_i, ix_f = self.get_excitation_indices()
        ...
        for px, py, col, lbl in self.held_pfy:
            if lbl != active_label: self.ax_pfy.plot(px, py, color=col, alpha=0.6, linestyle='--', label=f"{lbl} (Held)")
        ...
        self.ax_map.axvline(xs, color='cyan', linestyle='--', linewidth=1); self.ax_map.axvline(xe, color='cyan', linestyle='--', linewidth=1); self.canvas.draw()
    ...
    def export_2d_map(self):
        ...
        if not path: return
        try:
            dict_out = {'rixs_sum': {'rixs_plane0_norm': np.array([self.current_data['z_norm']]), 'valid_nominal_mono': self.current_data['x'], 'bin_centers': self.current_data['y'], 'sample_name': 'Summed', 'sample_id': 'SUM'}}
            with gzip.open(path, 'wb') as f: cPickle.dump(dict_out, f, protocol=4)
